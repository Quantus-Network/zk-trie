...
/// The proof contains the storage values so that the partial storage backend
/// can be reconstructed by a verifier that does not already have access to the
/// key-value pairs.
///
/// The proof consists of the set of serialized nodes in the storage trie
/// accessed when looking up the keys covered by the proof. Verifying the proof
/// requires constructing the partial trie from the serialized nodes and
/// performing the key lookups.
#[derive(Debug, PartialEq, Eq, Clone, Encode, Decode, TypeInfo)]
pub struct StorageProof {
    trie_nodes: BTreeSet<Vec<u8>>,
}
impl StorageProof {
    /// Constructs a storage proof from a subset of encoded trie nodes in a
    /// storage backend.
    ...
    /// storage backend.
    ///
    /// Returns an error if the provided subset of encoded trie nodes contains
    /// duplicates.
    ...
    ///
    /// An empty proof is capable of only proving trivial statements (ie. that
    /// an empty set of key-value pairs exist in storage).
    pub fn empty() -> Self {
        StorageProof {
    ...
    /// Convert into an iterator over encoded trie nodes in lexicographical
    /// order constructed from the proof.
    pub fn into_iter_nodes(self) -> impl Sized + DoubleEndedIterator<Item = Vec<u8>> {
        self.trie_nodes.into_iter()
    }
    /// Create an iterator over encoded trie nodes in lexicographical order
    ...
    /// Merges multiple storage proofs covering potentially different sets of
    /// keys into one proof covering all keys. The merged proof output may
    /// be smaller than the aggregate size of the input proofs due to
    /// deduplication of trie nodes.
    pub fn merge(proofs: impl IntoIterator<Item = Self>) -> Self {
        let trie_nodes =
            proofs.into_iter().flat_map(|proof| proof.into_iter_nodes()).collect::<BTreeSet<_>>().into_iter().collect();
        Self {
            trie_nodes,
        }
    }
    ...
    ///
    /// Running this operation is a slow operation (build the whole compact
    /// proof) and should only be in non sensitive path.
    ///
...
impl CompactProof {
    ...
    pub fn to_storage_proof<H: Hasher>(
        ...
        let root = crate::decode_compact::<Layout<H>, _, _>(&mut db, self.iter_compact_encoded_nodes(), expected_root)?;
        Ok((
