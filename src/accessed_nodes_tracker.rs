...
/// Helper struct used to ensure that a storage proof doesn't contain duplicate
/// or unused nodes.
///
/// The struct needs to be used as a `TrieRecorder` and
/// `ensure_no_unused_nodes()` has to be called to actually perform the check.
pub struct AccessedNodesTracker<H: Hash> {
    proof_nodes_count: usize,
    recorder: BTreeSet<H>,
}
impl<H: Hash> AccessedNodesTracker<H> {
    /// Create a new instance of `RedundantNodesChecker`, starting from a
    /// `RawStorageProof`.
    pub fn new(proof_nodes_count: usize) -> Self {
        Self {
            proof_nodes_count,
            recorder: BTreeSet::new(),
        }
    }
...
pub mod tests {
    ...
    const TEST_DATA: &[(&[u8], &[u8])] = &[(b"key1", &[1; 64]), (b"key2", &[2; 64]), (b"key3", &[3; 64])];
    #[test]
    fn proof_with_unused_nodes_is_rejected() {
        ...
        let mut accessed_nodes_tracker = AccessedNodesTracker::<Hash>::new(proof_nodes_count);
        {
            ...
            let trie = TrieDBBuilder::<Layout>::new(&db, &root).with_recorder(&mut accessed_nodes_tracker).build();
