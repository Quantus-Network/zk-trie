...
/// ! Provides the [`SharedNodeCache`], the [`SharedValueCache`] and the
/// [`SharedTrieCache`] ! that combines both caches and is exported to the
/// outside.
...
pub struct SharedNodeCacheLimiter {
    ...
    ///
    /// This space is always consumed whether there are any items in the map or
    /// not.
    ...
    H: AsRef<[u8]>,
{
    ...
    fn is_over_the_limit(&self, _length: usize) -> bool {
        // Once we hit the limit of max items evicted this will return `false` and
        // prevent any further evictions, but this is fine because the outer
        // loop which inserts items into this cache will just detect this and
        // stop inserting new items.
...
type SharedNodeCacheMap<H> = LruMap<H, NodeOwned<H>, SharedNodeCacheLimiter, schnellru::RandomState>;
/// The shared node cache.
///
/// Internally this stores all cached nodes in a [`LruMap`]. It ensures that
/// when updating the cache, that the cache stays within its allowed bounds.
...
impl<H: AsRef<[u8]> + Eq + std::hash::Hash> SharedNodeCache<H> {
    ...
    /// Update the cache with the `list` of nodes which were either newly added
    /// or accessed.
    pub fn update(&mut self, list: impl IntoIterator<Item = (H, NodeCached<H>)>) {
        let mut access_count = 0;
        let mut add_count = 0;
        self.lru.limiter_mut().items_evicted = 0;
        self.lru.limiter_mut().max_items_evicted = self.lru.len() * 100 / super::SHARED_NODE_CACHE_MAX_REPLACE_PERCENT;
...
impl ValueCacheKeyHash {
    pub fn from_hasher_and_storage_key(mut hasher: impl std::hash::Hasher, storage_key: &[u8]) -> Self {
...
pub(super) struct ValueCacheKey<H> {
    ...
    /// The hash that identifies this instance of `storage_root` and
    /// `storage_key`.
...
impl<H> ValueCacheKey<H> {
    ...
    /// Checks whether the key is equal to the given `storage_key` and
    /// `storage_root`.
    #[inline]
...
type SharedValueCacheMap<H> =
    schnellru::LruMap<ValueCacheKey<H>, CachedValue<H>, SharedValueCacheLimiter, BuildNoHashHasher<ValueCacheKey<H>>>;
...
impl<H: Eq + std::hash::Hash + Clone + Copy + AsRef<[u8]>> SharedValueCache<H> {
    ...
    ///
    /// The `added` values are the ones that have been collected by doing
    /// operations on the trie and now should be stored in the shared cache.
    /// The `accessed` values are only referenced by the
    /// [`ValueCacheKeyHash`] and represent the values that were retrieved from
    /// this shared cache. These `accessed` values are being put to the
    /// front of the internal [`LruMap`] like the `added` ones.
    pub fn update(
        &mut self,
        ...
        for hash in accessed {
            ...
            //
            // Since we are only comparing the hashes here it may lead us to promoting the
            // wrong values as the most recently accessed ones. However this is
            // harmless as the only consequence is that we may accidentally
            // prune a recently used value too early.
        ...
        // evicting items which we've added ourselves in previous iterations of this
        // loop.
        self.lru.limiter_mut().items_evicted = 0;
        self.lru.limiter_mut().max_items_evicted = self.lru.len() * 100 / super::SHARED_VALUE_CACHE_MAX_REPLACE_PERCENT;
...
///
/// It should be instantiated once per node. It will hold the trie nodes and
/// values of all operations to the state. To not use all available memory it
/// will ensure to stay in the bounds given via the [`CacheSize`] at startup.
///
...
impl<H: Hasher> SharedTrieCache<H> {
    ...
    pub fn new(cache_size: CacheSize) -> Self {
        ...
        // Calculate how much memory the maps will be allowed to hold inline given our
        // budget.
        let value_cache_max_inline_size =
            SharedValueCacheMap::<H::Out>::memory_usage_for_memory_budget(value_cache_inline_budget);
        ...
        Self {
            inner: Arc::new(RwLock::new(SharedTrieCacheInner {
                node_cache: SharedNodeCache::new(node_cache_max_inline_size, node_cache_max_heap_size),
                value_cache: SharedValueCache::new(value_cache_max_inline_size, value_cache_max_heap_size),
            })),
        }
    }
    /// Create a new [`LocalTrieCache`](super::LocalTrieCache) instance from
    /// this shared cache.
    ...
    pub fn used_memory_size(&self) -> usize {
        ...
        let value_cache_size = inner.value_cache.lru.memory_usage() + inner.value_cache.lru.limiter().heap_size;
        let node_cache_size = inner.node_cache.lru.memory_usage() + inner.node_cache.lru.limiter().heap_size;
    ...
    #[cfg(test)]
    pub(super) fn read_lock_inner(&self) -> parking_lot::RwLockReadGuard<'_, SharedTrieCacheInner<H>> {
        self.inner.read()
    }
    /// Returns the write locked inner.
    pub(super) fn write_lock_inner(&self) -> Option<RwLockWriteGuard<'_, SharedTrieCacheInner<H>>> {
        // This should never happen, but we *really* don't want to deadlock. So let's
        // have it timeout, just in case. At worst it'll do nothing, and at best
        // it'll avert a catastrophe and notify us that there's a problem.
...
mod tests {
    ...
    fn shared_value_cache_works() {
        ...
        assert_eq!(
            ...
            Arc::strong_count(cache.lru.limiter_mut().known_storage_keys.get_key_value(&key[..]).unwrap().0)
        );
        ...
        // Just accessing a key should not change anything on the size and number of
        // entries.
        cache.update(vec![], vec![ValueCacheKey::hash_data(&key[..], &root0)]);
        assert_eq!(1, cache.lru.limiter_mut().known_storage_keys.len());
        assert_eq!(3, Arc::strong_count(cache.lru.limiter_mut().known_storage_keys.get_key_value(&key[..]).unwrap().0));
        assert_eq!(key.len(), cache.lru.limiter().heap_size);
        assert_eq!(cache.lru.len(), 2);
        ...
        assert!(cache.lru.limiter().heap_size <= cache.lru.limiter().max_heap_size);
        assert_eq!(cache.lru.limiter().heap_size, 10);
        // Updating the cache again with exactly the same data should not change
        // anything.
        cache.update(
            vec![
                (ValueCacheKey::new_value(&key[..], root1), CachedValue::NonExisting),
        ...
        cache.update(vec![(ValueCacheKey::new_value(vec![10; 10], root0), CachedValue::NonExisting)], vec![]);
        assert!(cache.lru.limiter_mut().known_storage_keys.get_key_value(&key[..]).is_none());
    }
}
