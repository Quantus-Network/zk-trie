...
//! [`LocalTrieCache`] and [`TrieCache`]. The [`SharedTrieCache`] is the
//! instance that should be kept around for the entire lifetime of the node. It
//! will store all cached trie nodes and values on a global level. Then there is
//! the [`LocalTrieCache`] that should be kept around per state instance
//! requested from the backend. As there are very likely multiple accesses to
//! the state per instance, this [`LocalTrieCache`] is used to cache the nodes
//! and the values before they are merged back to the shared instance. Last but
//! not least there is the [`TrieCache`] that is being used per access to
//! the state. It will use the [`SharedTrieCache`] and the [`LocalTrieCache`] to
//! fulfill cache requests. If both of them don't provide the requested data it
//! will be inserted into the [`LocalTrieCache`] and then later into the
//! [`SharedTrieCache`].
//!
//! The [`SharedTrieCache`] is bound to some maximum number of bytes. It is
//! ensured that it never runs above this limit. However as long as data is
//! cached inside a [`LocalTrieCache`] it isn't taken into account when limiting
//! the [`SharedTrieCache`]. This means that for the lifetime of a
//! [`LocalTrieCache`] the actual memory usage could be above the allowed
//! maximum.
use std::{
    collections::HashMap,
    sync::{
...
/// The maximum amount of time we'll wait trying to acquire the shared cache
/// lock when the local cache is dropped and synchronized with the share cache.
///
...
/// The maximum number of existing keys in the shared cache that a single local
/// cache can promote to the front of the LRU cache in one go.
///
/// If we have a big shared cache and the local cache hits all of those keys we
/// don't want to spend forever bumping all of them.
...
/// The maximum size of the memory allocated on the heap by the local cache, in
/// bytes.
///
/// The size of the node cache should always be bigger than the value cache. The
/// value cache is only holding weak references to the actual values found in
/// the nodes and we account for the size of the node as part of the node cache.
...
/// A limiter for the local node cache. This makes sure the local cache doesn't
/// grow too big.
#[derive(Default)]
    ...
    H: AsRef<[u8]> + std::fmt::Debug,
{
    ...
    #[inline]
    fn on_insert<'a>(&mut self, _length: usize, key: H, cached_node: NodeCached<H>) -> Option<(H, NodeCached<H>)> {
    ...
    fn on_replace(
        ...
        self.current_heap_size = self.current_heap_size + new_node.heap_size() - old_node.heap_size();
        true
    }
    #[inline]
...
/// A limiter for the local value cache. This makes sure the local cache doesn't
/// grow too big.
#[derive(Default)]
...
/// A struct to gather hit/miss stats to aid in debugging the performance of the
/// cache.
#[derive(Default)]
struct HitStats {
...
type ValueCacheMap<H> =
    LruMap<ValueCacheKey<H>, CachedValue<H>, LocalValueCacheLimiter, BuildNoHashHasher<ValueCacheKey<H>>>;
type ValueAccessSet = LruMap<ValueCacheKeyHash, (), schnellru::ByLength, BuildNoHashHasher<ValueCacheKeyHash>>;
/// The local trie cache.
///
/// This cache should be used per state instance created by the backend. One
/// state instance is referring to the state of one block. It will cache all the
/// accesses that are done to the state which could not be fulfilled by the
/// [`SharedTrieCache`]. These locally cached items are merged back to the
/// shared trie cache when this instance is dropped.
///
/// When using [`Self::as_trie_db_cache`] or [`Self::as_trie_db_mut_cache`], it
/// will lock Mutexes. So, it is important that these methods are not called
/// multiple times, because they otherwise deadlock.
pub struct LocalTrieCache<H: Hasher> {
    ...
    ///
    /// This will be used to ensure that these nodes are brought to the front of
    /// the lru when this local instance is merged back to the shared cache.
    /// This can actually lead to collision when two [`ValueCacheKey`]s with
    /// different storage roots and keys map to the same hash. However,
    /// as we only use this set to update the lru position it is fine, even if
    /// we bring the wrong value to the top. The important part is that we
    /// always get the correct value from the value cache for a given key.
...
impl<H: Hasher> LocalTrieCache<H> {
    ...
    ///
    /// The given `storage_root` needs to be the storage root of the trie this
    /// cache is used for.
    ...
    ///
    /// After finishing all operations with [`TrieDBMut`](trie_db::TrieDBMut)
    /// and having obtained the new storage root, [`TrieCache::merge_into`]
    /// should be called to update this local cache instance. If the
    /// function is not called, cached data is just thrown away and not
    /// propagated to the shared cache. So, accessing these new items will be
    /// slower, but nothing would break because of this.
...
enum ValueCache<'a, H: Hasher> {
    ...
    /// This is used for example when a new trie is being build, to cache new
    /// values.
...
impl<H: Hasher> ValueCache<'_, H> {
    ...
    fn get(&mut self, key: &[u8], shared_cache: &SharedTrieCache<H>, stats: &HitStats) -> Option<&CachedValue<H::Out>> {
        ...
        match self {
            ...
            Self::ForStorageRoot {
                ...
                // happens when the node is dropped that contains the strong reference to the
                // data.
                //
                ...
                if let Some(value) =
                    local_value_cache.peek_by_hash(hash.raw(), |existing_key, _| existing_key.is_eq(storage_root, key))
                {
                    stats.local_hits.fetch_add(1, Ordering::Relaxed);
                    return Some(value);
                }
...
///
/// If this instance was created for using it with a
/// [`TrieDBMut`](trie_db::TrieDBMut), it needs to be merged back into the
/// [`LocalTrieCache`] with [`Self::merge_into`] after all operations are done.
...
impl<'a, H: Hasher> TrieCache<'a, H> {
    ...
    ///
    /// This function is only required to be called when this instance was
    /// created through [`LocalTrieCache::as_trie_db_mut_cache`], otherwise
    /// this method is a no-op. The given `storage_root` is the new storage
    /// root that was obtained after finishing all operations
...
impl<'a, H: Hasher> trie_db::TrieCache<NodeCodec<H>> for TrieCache<'a, H> {
    fn get_or_insert_node(
        ...
        Ok(&node?.expect("you can always insert at least one element into the local cache; qed").node)
    }
    fn get_node(&mut self, hash: &H::Out) -> Option<&NodeOwned<H::Out>> {
        ...
        match cached_node {
            ...
            Ok(None) => {
                unreachable!("you can always insert at least one element into the local cache; qed");
            }
            Err(()) => None,
        }
    }
...
mod tests {
    ...
    fn basic_cache_works() {
        ...
        shared_cache
            ...
            .value_cache_mut()
            .lru
            .insert(ValueCacheKey::new_value(TEST_DATA[1].0, root), (fake_data.clone(), Default::default()).into());
        {
            let mut cache = local_cache.as_trie_db_cache(root);
            let trie = TrieDBBuilder::<Layout>::new(&db, &root).with_cache(&mut cache).build();
            // We should now get the "fake_data", because we inserted this manually to the
            // cache.
    ...
    fn trie_db_mut_cache_works() {
        ...
        let mut new_root = root;
        {
            ...
            {
                let mut trie =
                    TrieDBMutBuilder::<Layout>::from_existing(&mut db, &mut new_root).with_cache(&mut cache).build();
                trie.insert(&new_key, &new_value).unwrap();
            }
            cache.merge_into(&local_cache, new_root);
        }
        // After the local cache is dropped, all changes should have been merged back to
        // the shared cache.
        let cached_data = shared_cache
            .read_lock_inner()
            .value_cache()
    ...
    fn trie_db_mut_cache_and_recorder_work_together() {
        ...
        for i in 0..5 {
            ...
            {
                let mut trie = TrieDBMutBuilder::<Layout>::from_existing(&mut memory_db, &mut proof_root).build();
                for (key, value) in DATA_TO_ADD {
                    trie.insert(key, value).unwrap();
                }
            }
    ...
    fn cache_lru_works() {
        ...
        // The second run is using an empty value cache to ensure that we access the
        // nodes.
        for _ in 0..2 {
            {
                let local_cache = shared_cache.local_cache();
            ...
            // Ensure that the accessed items are most recently used items of the shared
            // value cache.
            assert!(shared_cache
                .read_lock_inner()
                .value_cache()
        ...
        let most_recently_used_nodes =
            shared_cache.read_lock_inner().node_cache().lru.iter().map(|d| *d.0).collect::<Vec<_>>();
        {
            let local_cache = shared_cache.local_cache();
            let mut cache = local_cache.as_trie_db_cache(root);
            let trie = TrieDBBuilder::<Layout>::new(&db, &root).with_cache(&mut cache).build();
