...
impl<H: Hasher> Drop for LocalTrieCache<H> {
    fn drop(&mut self) {
        ...
        shared_inner
            .node_cache_mut()
            .update(self.node_cache.get_mut().drain());

        shared_inner.value_cache_mut().update(
            self.value_cache.get_mut().drain(),
            self.shared_value_cache_access
                .get_mut()
                .drain()
                .map(|(key, ())| key),
        );
...
impl<'a, H: Hasher> trie_db::TrieCache<NodeCodec<H>> for TrieCache<'a, H> {
    ...
    fn lookup_value_for_key(&mut self, key: &[u8]) -> Option<&CachedValue<H::Out>> {
        let res = self
            .value_cache
            .get(key, &self.shared_cache, &self.stats.value_cache);
    ...
    fn basic_cache_works() {
...

        {
            ...
            let trie = TrieDBBuilder::<Layout>::new(&db, &root)
                ...
                .build();
            assert_eq!(
                TEST_DATA[0].1.to_vec(),
                trie.get(TEST_DATA[0].0).unwrap().unwrap()
            );
        }
        ...
        let cached_data = shared_cache
            ...
            .clone();
        assert_eq!(
            Bytes::from(TEST_DATA[0].1.to_vec()),
            cached_data.data().flatten().unwrap()
        );
...

        {
            ...
            assert_eq!(
                b"fake_data".to_vec(),
                trie.get(TEST_DATA[1].0).unwrap().unwrap()
            );
        }
    }
