...
impl<H: Hasher> Recorder<H> {
    ...
    pub fn commit_transaction(&self) -> Result<(), ()> {
        ...
        if let Some(parent_transaction) = inner.transactions.last_mut() {
            parent_transaction
                .accessed_nodes
                .extend(transaction.accessed_nodes);
...
impl<'a, H: Hasher> trie_db::TrieRecorder<H::Out> for TrieRecorder<'a, H> {
    fn record(&mut self, access: TrieAccess<H::Out>) {
        ...
        self.encoded_size_estimation
            .fetch_add(encoded_size_update, Ordering::Relaxed);
    }
    ...
    fn recorder_transactions_rollback_work() {
        ...
        for i in 0..4 {
            recorder.start_transaction();
            {
                ...
                assert_eq!(
                    TEST_DATA[i].1.to_vec(),
                    trie.get(TEST_DATA[i].0).unwrap().unwrap()
                );
            }
        ...
        for i in 0..5 {
            ...
            for a in 0..4 {
                if a < 4 - i {
                    assert_eq!(
                        TEST_DATA[a].1.to_vec(),
                        trie.get(TEST_DATA[a].0).unwrap().unwrap()
                    );
                } else {
