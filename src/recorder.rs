...
//! Trie recorder
//!
//! Provides an implementation of the [`TrieRecorder`](trie_db::TrieRecorder)
//! trait. It can be used to record storage accesses to the state to generate a
//! [`StorageProof`].
use std::{
...
struct Transaction<H> {
    ...
    ///
    /// For each transaction we only store the `storage_root` and the old states
    /// per key. `None` state means that the key wasn't recorded before.
...
struct RecorderInner<H> {
    /// The keys for that we have recorded the trie nodes and if we have
    /// recorded up to the value.
    ///
...
/// proof and to provide transaction support. The `as_trie_recorder` method
/// provides a [`trie_db::TrieDB`] compatible recorder that implements the
/// actual recording logic.
pub struct Recorder<H: Hasher> {
    inner: Arc<Mutex<RecorderInner<H::Out>>>,
    /// The estimated encoded size of the storage proof this recorder will
    /// produce.
    ///
    /// We store this in an atomic to be able to fetch the value while the
    /// `inner` is may locked.
...
impl<H: Hasher> Recorder<H> {
    ...
    /// Returns the recorder as [`TrieRecorder`](trie_db::TrieRecorder)
    /// compatible type.
    ///
    /// - `storage_root`: The storage root of the trie for which accesses are
    ///   recorded. This is important when recording access to different tries
    ///   at once (like top and child tries).
    ///
    /// NOTE: This locks a mutex that stays locked until the return value is
    /// dropped.
    #[inline]
    ...
    ///
    /// While a recorder can be cloned, all share the same internal state. After
    /// calling this function, all other instances will have their internal
    /// state reset as well.
    ///
    /// If you don't want to drain the recorded state, use
    /// [`Self::to_storage_proof`].
    ///
    ...
    ///
    /// In contrast to [`Self::drain_storage_proof`] this doesn't consume and
    /// doesn't clear the recordings.
    ///
    /// Returns the [`StorageProof`].
    ...
    ///
    /// The estimation is based on all the nodes that were accessed until now
    /// while accessing the trie.
    ...
    pub fn rollback_transaction(&self) -> Result<(), ()> {
        let mut inner = self.inner.lock();
        // We locked `inner` and can just update the encoded size locally and then store
        // it back to the atomic.
...
impl<'a, H: Hasher> TrieRecorder<'a, H> {
    ...
    fn update_recorded_keys(&mut self, full_key: &[u8], access: RecordedForKey) {
        ...
        let entry = inner.recorded_keys.entry(self.storage_root).or_default().entry(full_key.into());
        let key = entry.key().clone();
        // We don't need to update the record if we only accessed the `Hash` for the
        // given `full_key`. Only `Value` access can be an upgrade from `Hash`.
        let entry = if matches!(access, RecordedForKey::Value) {
            entry.and_modify(|e| {
                if let Some(tx) = inner.transactions.last_mut() {
                    ...
                    tx.recorded_keys.entry(self.storage_root).or_default().entry(key.clone()).or_insert(Some(*e));
                }
                *e = access;
            })
        } else {
        ...
        entry.or_insert_with(|| {
            if let Some(tx) = inner.transactions.last_mut() {
                ...
                tx.recorded_keys.entry(self.storage_root).or_default().entry(key).or_insert(None);
            }
            access
        });
    }
    ...
    fn record(&mut self, access: TrieAccess<H::Out>) {
        ...
        match access {
            ...
            TrieAccess::InlineValue {
                ...
                // A value was accessed that is stored inline a node and we recorded all trie
                // nodes to access this value.
                self.update_recorded_keys(full_key, RecordedForKey::Value);
            }
        };
...
mod tests {
    ...
    fn recorder_works() {
        ...
        let recorder = Recorder::default();
        {
            ...
            let trie = TrieDBBuilder::<Layout>::new(&db, &root).with_recorder(&mut trie_recorder).build();
    ...
    impl RecorderStats {
        fn extract(recorder: &Recorder) -> Self {
            ...
            let recorded_keys = inner.recorded_keys.iter().flat_map(|(_, keys)| keys.keys()).count();
            Self {
                recorded_keys,
    ...
    fn recorder_transaction_accessed_keys_works() {
        ...
        recorder.start_transaction();
        {
            let mut trie_recorder = recorder.as_trie_recorder(root);
            let trie = TrieDBBuilder::<Layout>::new(&db, &root).with_recorder(&mut trie_recorder).build();
            assert_eq!(sp_core::Blake2Hasher::hash(TEST_DATA[0].1), trie.get_hash(TEST_DATA[0].0).unwrap().unwrap());
        ...
        recorder.start_transaction();
        {
            ...
            let trie = TrieDBBuilder::<Layout>::new(&db, &root).with_recorder(&mut trie_recorder).build();
            assert_eq!(TEST_DATA[0].1.to_vec(), trie.get(TEST_DATA[0].0).unwrap().unwrap());
            assert!(matches!(trie_recorder.trie_nodes_recorded_for_key(key), RecordedForKey::Value,));
        }
        recorder.rollback_transaction().unwrap();
        {
            let trie_recorder = recorder.as_trie_recorder(root);
